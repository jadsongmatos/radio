...
function buildPromptFromArtist(artistName: string) {
  ...
  // Prompt do LB Radio no estilo “Troi”: funciona bem com artist:(...)
  return `artist:(${a})`
}
...
async function ensurePrefill(prisma: any) {
  ...
  // fallback ultra simples caso LB falhe/venha vazio e a fila esteja zerada:
...
/**
 * Entrega o mais antigo (deleteAt=null) e marca deleteAt=now.
 * Se não houver pendentes, tenta “replay” do último item do DB (sem marcar nada).
 */
async function deliverOne(prisma: any) {
  ...
  server: {
    handlers: {
      GET: async () => {
        ...
        if (!next?.youtubeUrl) {
          // vazio -> liquidsoap devolve [] e fallback assume
          return new Response('', {
            status: 204,
        ...
        // já devolve com prefixo youtube-dl: (AzuraCast/yt-dlp)
